...
void DW1000RangingClass::transmitRangeReport(DW1000Device* myDistantDevice) {
    ...
    // 1. 產生標準 MAC 表頭 [cite: 163]
    _globalMac.generateShortMACFrame(data, _currentShortAddress, myDistantDevice->getByteShortAddress());
    data[SHORT_MAC_LEN] = RANGE_REPORT; // 功能碼 0x03 [cite: 163]
    
    // 2. 準備距離數據字串 [cite: 164]
    float dist = myDistantDevice->getRange();
    char plainBuf[128]; 
    int baseLen = snprintf(plainBuf, sizeof(plainBuf), "%.4f", dist); // 保留4位小數 [cite: 165]

    int offset = SHORT_MAC_LEN + 1;

    // --- 核心邏輯：區分 90 bytes 與 45 bytes --- [cite: 172, 180]
    if (_isEncryptionEnabled) {
        // ====== 分支 A: 加密模式 (強制 90 bytes) ====== [cite: 172]
        
        // 1) 補足 Padding 讓總長度達到 90 [cite: 166, 167]
        int targetPadding = 45; 
        if (baseLen + targetPadding < sizeof(plainBuf)) {
            memset(plainBuf + baseLen, '0', targetPadding);
            baseLen += targetPadding;
            plainBuf[baseLen] = '\0';
        }

        // 2) 準備 IV 與 AES-GCM 加密 [cite: 168, 170, 173]
        unsigned char iv[ENC_IV_LEN];
        memset(iv, 0, ENC_IV_LEN);
        memcpy(iv, &_expIVCounter, sizeof(uint32_t));
        _expIVCounter++; // 遞增 IV 避免重放攻擊 [cite: 169]
        ...
        mbedtls_gcm_setkey(&gcm, MBEDTLS_CIPHER_ID_AES, UWB_AES_KEY, 256); // 使用 256 bits key [cite: 171]
        ...
        // 3) 組裝加密封包 [cite: 174, 175]
        data[offset++] = ENC_VER; // 0x01 代表加密 [cite: 174]
        ...
        // 4) 關鍵修改：強制告訴晶片發送 90 bytes [cite: 180]
        copyShortAddress(_lastSentToShortAddress, myDistantDevice->getByteShortAddress());
        DW1000.setData(data, 90); 
        
    } else {
        // ====== 分支 B: 明文模式 (強制 45 bytes) ====== [cite: 178]
        
        data[offset++] = 0x00; // 0x00 代表明文 [cite: 178]
        memcpy(&data[offset], plainBuf, baseLen);
        offset += baseLen;
        
        // 關鍵修改：強制告訴晶片發送 45 bytes [cite: 180]
        copyShortAddress(_lastSentToShortAddress, myDistantDevice->getByteShortAddress());
        DW1000.setData(data, 45); 
    }
